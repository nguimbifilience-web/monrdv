...
<div class="p-8">
    <div class="flex items-center justify-between mb-6">
        <div>
            <h1 class="text-3xl font-black text-blue-900 uppercase italic">Feuilles d'examen</h1>
            <p class="text-xs text-gray-400 font-bold uppercase tracking-widest">Demandes d'examens que vous avez prescrites</p>
        </div>
        <a href="{{ route('medecin.examens.create') }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-2xl font-bold text-xs uppercase tracking-widest shadow-md shadow-blue-100">
            ...
            <tbody class="divide-y divide-gray-50">
                @forelse($feuilles as $f)
                    <tr class="hover:bg-gray-50/30 transition-colors">
                        ...
                        <td class="p-5 text-xs font-black text-blue-900 uppercase">{{ $f->patient->nom }} {{ $f->patient->prenom }}</td>
                ...
                @empty
                    <tr><td colspan="4" class="p-8 text-center text-xs text-gray-400 uppercase tracking-widest">Aucune feuille d'examen</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>

    <div class="mt-4">{{ $feuilles->links() }}</div>
</div>
@endsection
